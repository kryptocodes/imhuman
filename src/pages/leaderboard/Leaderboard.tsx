      ...
      <div className=" flex flex-col gap-2 mt-4">
        {
          leaderboardData?.map((data:any, i) => {
            if (i < 3) return
            return (
              <div className={` rounded-2xl flex justify-between items-center ${data?.walletAddress === account.address ? 'bg-brand text-white' : 'bg-white text-brand'}  border-2 py-4 px-3 `} key={i} >
                <div className="flex gap-2">
                  <div className=" rounded-full text-brand bg-[#ebeafd] flex items-center text-xs font-bold px-4 ">{i + 1}</div>
                  <div className="flex items-center gap-2">
                    <div className={` ${data?.walletAddress === account.address ? 'bg-white' : 'bg-brand'} rounded-full w-8 h-8 overflow-hidden `}>
                        <img src={`https://api.dicebear.com/9.x/identicon/svg?seed=${data?.walletAddress}`} alt="" className="w-8 h-8" />
                    </div>
