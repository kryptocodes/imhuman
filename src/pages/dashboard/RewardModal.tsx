...
const RewardModal: FC<RewardModalProps> = (reward) => {
  ...
  const handleClaim = async () => {
    ...
    try {
      await axios.post(
        "https://m8aanm1noe.execute-api.ap-southeast-1.amazonaws.com/api/reward/claim",
        {
  ...
  return (
    <>
      <button
        ...
        className="bg-white py-1 px-3 rounded-full font-bold cursor-pointer text-brand"
      >
        Claim
      </button>
      <AnimatePresence>
        ...
        {openModal ? (
          <div className=" fixed inset-0 grid place-items-center z-50">
            <motion.div
              ...
              exit={{ opacity: 0 }}
              ref={ref}
              className=" h-fit w-[320px] bg-white border-2 border-[#9EA8FD] rounded-3xl px-[10px]  py-3 "
            >
              ...
              <h2 className=" text-2xl font-bold text-black mt-4 ">
                {reward.reward.rewardType === "nft"
                  ? "You are a certified human"
                  : "Congratulations Human!"}
              </h2>
              <p className=" text-brand mt-2 text-xs ">
                {reward.reward.rewardType === "nft"
                  ? "In a world where AI and bots are reigning, you stand out as a real human being"
                  : "You've won your first Airdrop. This will be the first of many."}
              </p>
              <button
                className=" mt-4 overflow-hidden py-[14px] bg-brand text-white font-semibold w-full rounded-2xl "
