...
const TaskModal: FC<TaskModalProps> = ({ task, isCompleted }) => {
  ...
  const handleTask = async () => {
    ...
    navigator.userAgent.match(/(iPod|iPhone|iPad)/)
      ? window.open(req.requestUrl, "_top")
  ...
  return (
    <>
      <div
        ...
        className="bg-white p-3 rounded-3xl flex justify-between items-center font-bold cursor-pointer text-brand"
      >
        <div className="flex gap-2 items-center">
          <div className=" bg-brand rounded-xl h-[54px] w-[54px] overflow-hidden  ">
            <img
              src={`https://api.dicebear.com/9.x/shapes/svg?seed=${task.description}`}
              alt=""
            />
          </div>
